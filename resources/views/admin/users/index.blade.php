                    ...
                    @forelse($data as $users)
                        ...
                        <tr>
                            ...
                            <td class="px-6 py-4 whitespace-nowrap imgthumb"> 
                                @if($users->getFirstMediaUrl('avatar_user','thumb'))
                                <img src="{{ $users->getFirstMediaUrl('avatar_user','thumb') }}" alt="" srcset="">
                                @endif
                            </td>
                            <td class="px-6 py-4 whitespace-nowrap">{{$users->name}}</td>
                            <td class="px-6 py-4 whitespace-nowrap">{{$users->email}}</td>
                            <td class="px-6 py-4 whitespace-nowrap">{{$users->role->name}}</td>
                            <td class="px-6 py-4 whitespace-nowrap">
                                @forelse($users->filiale as $pays)   
                            ...
                            <td class="px-6 py-4 whitespace-nowrap">
                                <a href="{{route('users.edit', [$users->id])}}" class="text-blue-500"><i class="fa fa-pencil"></i></a>
                            </td>
                        
                            <td class="px-6 py-4 whitespace-nowrap">
                                <form method="post" action="{{route('users.destroy', [$users->id])}}" id="deleteForm{{$users->id}}">
                                    @csrf
                                    @method('DELETE')
                                    <button type="submit" class="text-red-500" onclick="event.preventDefault(); if(confirm('Êtes-vous sûr de vouloir supprimer?')) {document.getElementById('deleteForm{{$users->id}}').submit();} else {return false;} ">Supprimer</button>
                                </form>
                            </td>
                        </tr>
                    @php($index++)
