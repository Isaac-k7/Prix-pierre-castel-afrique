<x-app-layout>
    ...
    <div >
        ...
        <div class="grid justify-items-stretch my-3">
            <a href="{{route('pays.create')}}" class="inline-flex items-center px-4 py-2 bg-gray-800 border border-transparent rounded-md font-semibold text-xs text-white uppercase tracking-widest hover:bg-gray-700 active:bg-gray-900 focus:outline-none focus:border-gray-900 focus:ring ring-gray-300 disabled:opacity-25 transition ease-in-out duration-150 flex justify-self-end">{{_('Ajouter un pays')}}</a>
        </div>
        ...
        <table class="min-w-full divide-y divide-gray-200" id="tableau">
            ...
            @forelse($pays as $post)
                ...
                <!--     <td class="px-6 py-4 whitespace-nowrap">
                    ...
                    <td class="px-6 py-4 whitespace-nowrap">{{$post->code}}</td>
                    <td class="px-6 py-4 whitespace-nowrap">
                        <a href="{{route('pays.edit', [encrypt($post->id)])}}" class="text-blue-500"><i class="fa fa-pencil"></i></a>
                    </td>
                    <td class="px-6 py-4 whitespace-nowrap">
                        <form method="post" action="{{route('pays.destroy', [$post->id])}}" id="deleteForm{{$post->id}}">
                            @csrf
                            @method('DELETE')
